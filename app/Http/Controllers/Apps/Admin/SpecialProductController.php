...
class SpecialProductController extends Controller
{
    ...
    {
        $data = SpecialProduct::all();
        return view('admin.product-management.special_products.index',compact('data'));
    }
    ...
    public function edit(string $id)
    {
        ...
        return view('admin.product-management.special_products.edit',compact('data'));
    }
    ...
    public function update(Request $request, string $id)
    {
        try {
            $data =  SpecialProduct::find($id);
            if (!isset($data)) {
                return redirect()->route('product-management.special-products.index')->with(['message'=>'Data Not Found','type'=>'error']);
            }
            if ($request->title) {
                $input['title']  = $request->title;
            }
            ...
            return redirect()
            ->route('product-management.special-products.index')
    ...
    public function change_status(Request $request)
    {
        try {
            //code...
            $statusChange = SpecialProduct::where('id',$request->id)->update(['status'=>$request->status]);
            if($statusChange)
            {
